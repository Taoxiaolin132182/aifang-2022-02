...
def tar_job():
    print("打包执行文件前\n检查相关路径是否存在")
    if os.path.exists(cfg_dc.path_ori_file):
        if not os.path.exists(cfg_dc.path_com_file):
            os.mkdir(cfg_dc.path_com_file)
            time.sleep(0.1)
            cmd_chmod1 = "chmod -R 777 " + cfg_dc.path_com_file + " &"
            os.system(cmd_chmod1)
        print("开始打包-解密程序")
        #  cxfreeze /mnt/data/data/2test_remove/d_decode/decode_model.py --target-dir /mnt/data/data/2test_remove/try_decode
        code1 = "cxfreeze " + cfg_dc.path_ori_file + " --target-dir " + cfg_dc.path_com_file
        os.system(code1)
        # tar -zcPf try_decode_0908.tar.gz /mnt/data/data/2test_remove/try_decode/
        code2 = "tar -zcPf " + cfg_dc.tar_name + " " + cfg_dc.path_com_file
        os.system(code2)
        print("打包完成")
        # code3 = ""
        # os.system(code3)
    else:
        print("文件：{}  不存在".format(cfg_dc.path_ori_file))
