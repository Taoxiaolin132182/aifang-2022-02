...
class AiHjRunService:
    ...
    def __init__(self):
        ...
        # 传送带速度
        self._conveyer_speed = aicfg.CONVEYER_SPEED
        # 抓手时间
        self._tongs_time = aicfg.TONGS_TIME
        # 抓手在横杆上的速度
        self._tongs_x_speed = aicfg.TONGS_X_SPEED
        # 设备间隔距离
        self._device_length = aicfg.DEVICE_LENTH
        # 拍照间隔时间
        self._take_photo_interval = aicfg.TAKE_PHOTO_INTERVAL
        ...
        self.list_zh1 = aicfg.Choose_type
        self.num_yixian1 = 0
        self.num_changeda = 0
        self.bool_xfz = False
        ...
        self.upload_nobox = None
        self.label_path = aicfg.Choose_type[-1]  # [1,2,3]
        self.batch_cotton = []
        self.ai_image_result_path = ""
        self.real_all_piont_put_2 = 0
        self.cam_num = []
        self.cam_num2 = [aicfg.CAMERA_DEVICE_TUPLE[0], aicfg.CAMERA_DEVICE_TUPLE[1], aicfg.CAMERA_DEVICE_TUPLE[2]]
    ...
    def frist_check(self):
        ...
        try:
            ...
            name3 = aicfg.MELSEC_CODE.ARM_XINHAO
            time.sleep(2)
    ...
    def pre_run_1(self):
        try:
            # os.system("modprobe -r bluedroid_pm")  # 执行 屏蔽蓝牙中断 的指令
            # logger.info("已执行 蓝牙中断 指令")
            # time.sleep(2)
            '''删除过大空间'''
            max_space = [500, 200]
    ...
    def mkdir_pic_path1(self):
        ...
        date_path2 = aicfg.Path_upload + date_path1  # 设备编号+ 当天日期
        ...
        self.upload_nobox = "/mnt/data/data/upload_image/" + date_path2 + "_nobox"
        ...
        if not os.path.exists(self.upload_nobox):
            os.makedirs(self.upload_nobox)
    ...
    def run_produce_point_data(self):  # 生产者（产生点数据）
        ...
        while True:
            try:
                ...
                # if data:
                #     start_time = time2long(data.get(aicfg.MELSEC_CODE.MELSEC_TIME_SEC),
                #                            data.get(aicfg.MELSEC_CODE.MELSEC_TIME_MILLSEC))
                #     self.speed2 = transform_conveyer_speed(data.get(aicfg.MELSEC_CODE.CSD_SPEED1))
                # else:
        ...
        while True:
            try:
                ...
                if self.bool_xfz:
                    ...
                    if plc_status:
                        sleep_status1 = plc_status.get(aicfg.MELSEC_CODE.STOP_ALL1)
                    ...
                    if tong_status:
                        ...
                        bool_zs1_1 = tong_status.get(aicfg.MELSEC_CODE.TONGS_STATUS1)  # 抓取中
                        bool_zs1_2 = tong_status.get(aicfg.MELSEC_CODE.HOMING_STATUS1)  # 归位中
                        bool_zs2_1 = tong_status.get(aicfg.MELSEC_CODE.TONGS_STATUS2)  # 抓取中
                        bool_zs2_2 = tong_status.get(aicfg.MELSEC_CODE.HOMING_STATUS2)  # 归位中
                        # bool_zs1_1 = 1
                        # bool_zs1_2 = 1
                        # bool_zs2_1 = 1
                        # bool_zs2_2 = 1
                    ...
                    if zhua_a != 0:  # 锁定成抓手1/2（起码有一个抓手空闲）
                        ...
                        x = point_record.get("worldX") - aicfg.DIFF_X  # X值加减参数
                        y = point_record.get("worldY") - aicfg.LEFT_DIFF_Y
                        print("point_record",point_record)
                        # ------------------------------------------------------------------------------------
                        if zhua_a == 1:
                            zhua_b = 0  # 一但进入判断，就解锁（不论抓不着还是去抓取）
                            x = x + aicfg.X1_LEN1
                            y = y - 0.25*speed_bmq1 + aicfg.Y1_LEN1  # 抓手1现场补偿值加大0mm,-5mm(加了连接板5mm)
                            ...
                            if float(spend_length + will_length) < y:  # 此时，传送带未走到位，才有抓到的条件
                                ...
                                current_time = time2long(data.get(aicfg.MELSEC_CODE.MELSEC_TIME_SEC),
                                                         data.get(aicfg.MELSEC_CODE.MELSEC_TIME_MILLSEC))

                                # 下抓时间/最终的时间点：t_PLC + y/speed +偏差值- (t2_arm - t1_arm)
                                # down_time = round(speed_bmq1*aicfg.T1_LEN1/1000 + aicfg.T1_LEN2, 3)
                                end_time = current_time + (y/speed_bmq1 + aicfg.T1_LEN1 - (arm_time_now - start_time))*1000
                        ...
                        # --------------------------------------------------------------------------------
                            ...
                            zhua_b = 0  # 一但进入判断，就解锁（不论抓不着还是去抓取）
                            x = x + aicfg.X2_LEN1
                            y = y + 607 - 0.25*speed_bmq1 + aicfg.Y2_LEN1 #抓手2与抓手1相距607mm+5mm(加了连接板5mm)
                            ...
                            if float(spend_length + will_length) < y:  # 此时，传送带未走到位，才有抓到的条件
                                ...
                                current_time = time2long(data.get(aicfg.MELSEC_CODE.MELSEC_TIME_SEC),
                                                         data.get(aicfg.MELSEC_CODE.MELSEC_TIME_MILLSEC))
                                # 下抓时间/最终的时间点：t_PLC + y/speed +偏差值 - (t2_arm - t1_arm)
                                end_time = current_time + (y / speed_bmq1 + aicfg.T2_LEN1 - (arm_time_now - start_time)) * 1000
                                sec, mill_sec = long2time(end_time)  # 毫秒转时间
    ...
    def judge_speed(self):
        bool_jud_sp1 = True
        old_speed = None
        name2 = aicfg.MELSEC_CODE.ARM_XINHAO

        while bool_jud_sp1:

            cds_speed = self.read_speed().get(aicfg.MELSEC_CODE.CSD_SPEED1)  # 取PLC上的传送带速度
            # cds_speed = 2000
            if cds_speed < 500:
    ...
    def record_err(self):
        while self.bool_lock_time:
            ...
            try:
                name11 = aicfg.MELSEC_CODE.ERR_STATUS  # "D112"#PLC 异常记录更新位（有变化才写入）2020——1212
                name12 = aicfg.MELSEC_CODE.ERR_RECORD  # "M500"#PLC 异常记录初始位2020——1212
                list_char = aicfg.ERR_LIST
    ...
    def connect_plc(self):
        ...
        num_plc1 = 0
        while bool_plc1:
            bResult = self._melsec_ctrl.open((aicfg.MELSEC_SERVER_IP, aicfg.MELSEC_SERVER_PORT))
            if "ssc" in aicfg.Path_upload:
                bResult1 = self._melsec_ctrl_frist.open((aicfg.MELSEC_SERVER_IP_F, aicfg.MELSEC_SERVER_PORT))
            if not bResult:
    ...
    def read_data(self, zs_numb2):  # （生产者，消费者各调用2次，读取PLC的返回数据）
        ...
        try:
            if zs_numb2 == 0 or zs_numb2 == 8:
                ...
                name01 = aicfg.MELSEC_CODE.CSD_SPEED1
                bReadData = self._melsec_ctrl.read_dword_data(name01, 1)
                result = self._melsec_ctrl.read_dword_data(aicfg.MELSEC_CODE.MELSEC_TIME_SEC, 2)  # 返回数组
                name2 = aicfg.MELSEC_CODE.ARM_XINHAO
                data2 = [self.num_changed1()]
                ...
                if result is not None:  # 有值
                    data = {
                            aicfg.MELSEC_CODE.MELSEC_TIME_SEC: result[0],
                            aicfg.MELSEC_CODE.MELSEC_TIME_MILLSEC: result[1],
                            name01: bReadData[0]
                    }
                ...
                if zs_numb2 == 1:
                    name3 = [aicfg.MELSEC_CODE.TONGS_X_SPEED1,
                             aicfg.MELSEC_CODE.TONGS_ORIGIN_COORDINATES1,
                             aicfg.MELSEC_CODE.TONS_LIMIT_AFTER_COORDINATES1,
                             aicfg.MELSEC_CODE.TONS_LIMIT_PRE_COORDINATES1,
                             aicfg.MELSEC_CODE.TONG_CURRENT_COORDINATES1]
                else:
                    name3 = [aicfg.MELSEC_CODE.TONGS_X_SPEED2,
                             aicfg.MELSEC_CODE.TONGS_ORIGIN_COORDINATES2,
                             aicfg.MELSEC_CODE.TONS_LIMIT_AFTER_COORDINATES2,
                             aicfg.MELSEC_CODE.TONS_LIMIT_PRE_COORDINATES2,
                             aicfg.MELSEC_CODE.TONG_CURRENT_COORDINATES2]
                result = self._melsec_ctrl.read_dword_data(aicfg.MELSEC_CODE.MELSEC_TIME_SEC, 2)  # 返回数组
                time_now = time.time()
    ...
    def read_tongs_status(self, zs_numb):  # 在消费者中被调用
        while self.bool_lock_time:
            ...
            self.lock.acquire()  # 线程锁
            try:
                name1 = aicfg.MELSEC_CODE.TONGS_STATUS1
                name2 = aicfg.MELSEC_CODE.HOMING_STATUS1
                name3 = aicfg.MELSEC_CODE.TONGS_STATUS2
                name4 = aicfg.MELSEC_CODE.HOMING_STATUS2
    ...
    def read_sleep_status(self):  # 在消费者中被调用
        while self.bool_lock_time:
            ...
            try:
                #和PLC连接的自增信号
                data2 = [self.num_changed1()]
                result_PLC_connect = self._melsec_ctrl.write_dword_data(aicfg.MELSEC_CODE.ARM_XINHAO, data2)  # 写入自检信号位-断连状态
                if not result_PLC_connect:
                    logger.info("写入自增连接信号失败:{}".format(result_PLC_connect))
                if data2[0] % 10 == 2:
                    #刷新传送带速度值 ，传给全局变量
                    speed_ReadData = self._melsec_ctrl.read_dword_data(aicfg.MELSEC_CODE.CSD_SPEED1, 1)
                ...
                name01 = aicfg.MELSEC_CODE.STOP_ALL1
    ...
    def read_choose_model(self):
        ...
        self.lock.acquire()  # 线程锁
        try:
            name01 = aicfg.MELSEC_CODE.CHOOSE_MODEL
            bReadData = self._melsec_ctrl.read_dword_data(name01, 1)  #
            if bReadData is not None:
                data = {
                    name01: bReadData[0]
    ...
    def read_batch(self, sign1):#初始化时 sign1=Fasle ，
        ...
        try:
            ...
            sign2 = True #默认去读写
            if sign1:
                result_sign = self._melsec_ctrl.read_bit_data(aicfg.MELSEC_CODE.SIGN_BATCH, 1)
                if result_sign[0] == 0:#当信号位为0，表示无变化时，不去读取大小批次
                    sign2 = False
            logger.info('读取批次号--PLC通讯--2')
            if sign2:
                result1_ascii = self._melsec_ctrl.read_word_data(aicfg.MELSEC_CODE.BIG_BATCH, 5)
                result1_char1 = self.ascii_to_char(result1_ascii)
                result2_ascii = self._melsec_ctrl.read_word_data(aicfg.MELSEC_CODE.SMALL_BATCH, 8)
    ...
    def write_data(self, sec, millsec, x, feedback_x, feedback_speed, zs_numb3):  # 最后2个，反馈x,反馈速度（消费者调用）
        while self.bool_lock_time:
            ...
            try:
                if zs_numb3 == 1:
                    name4 = aicfg.MELSEC_CODE.GRAB_TIME_SEC1
                else:
                    name4 = aicfg.MELSEC_CODE.GRAB_TIME_SEC2
                data = [sec, millsec, x, feedback_x, feedback_speed]
                result1 = self._melsec_ctrl.write_dword_data(name4, data)  # 抓取时间戳
                # result2 = self._melsec_ctrl.write_dword_data(aicfg.MELSEC_CODE.GRAB_TIME_MILLSEC, [millsec])
                if result1 is not None:
                    return True
                return False
            finally:
    ...
    def write_homing_data(self, zs_numb1):  # （消费者调用）
        while self.bool_lock_time:
            ...
            try:
                if zs_numb1 == 1:
                    name2 = aicfg.MELSEC_CODE.TONGS_HOMING1
                else:
                    name2 = aicfg.MELSEC_CODE.TONGS_HOMING2
    ...
    def write_sleep_data(self, num_onemt):  # （消费者调用）(向PLC写入休眠信号)
        while self.bool_lock_time:
            ...
            try:
                if num_onemt == 0:
                    name3 = aicfg.MELSEC_CODE.SLEEP_PLC
                    self._melsec_ctrl_frist.write_bit_data(name3, [1])
                    return True
                elif num_onemt == 2:
                    logger.info("写入M50，1")
                    name3 = aicfg.MELSEC_CODE.START_OK
                    self._melsec_ctrl.write_bit_data(name3, [1])
                    if "ssc" in aicfg.Path_upload:
                        self._melsec_ctrl_frist.write_bit_data(name3, [1])
                    return True
                elif num_onemt == 3:
                    name3 = aicfg.MELSEC_CODE.CHANGE_OK
                    self._melsec_ctrl.write_bit_data(name3, [1])
                    return True
                elif num_onemt == 4:
                    name3 = aicfg.MELSEC_CODE.CHANGE_OK
                    ...
                    name3 = aicfg.MELSEC_CODE.DO_ONE_TIME_PLC
                    self._melsec_ctrl.write_bit_data(name3, [1])
                    return True
...
# ####---------------相机拍照/相机回调/图片存放-------------
    ...
    def saveImageFile(self, astrSerial, aoImage, asaveImgAddrss):  # 存图片，底行代码
        if aicfg.MOCK_SAVE_IMAGE:
            time.sleep(aicfg.MOCK_SAVE_IMAGE_RESULT_TIME)  # sleep时间
            return True
    ...
    def process_ai_request(self, camera_key):
        position = aicfg.CAMERA_DEVICE_POSITION_MAP[camera_key]  # 相机对应的位置
        start_n = time.time()
    ...
    def processAiData(self, res_data, position):  # 返回像素点坐标，相机位置，世界坐标的字典 的列表#（生产者调用）
        ...
        if len(data_label) > 0: # 为空列表时，可以直接返回 arr = []
            ...
            for i_point in range(len(data_label)):
                ...
                if data_score[i_point] < aicfg.Suspected_threshold:#当分数小于怀疑值的时候，点不要
                    continue
                ...
                if data_label[i_point] in self.list_zh1[int(self.choose_model1)]: # 当 label 是在PLC传来的分类列表中
                    ...
                    if data_label[i_point] == "yixian":
                        if float(data_score[i_point]) >= aicfg.Classification_threshold:
...
# ####---------------暂不使用-------------
    ...
    def read_tongs_speed(self):  # （未使用）
        self.lock.acquire()
        try:
            speed = self._melsec_ctrl.read_dword_data(aicfg.MELSEC_CODE.TONGS_X_SPEED1, 1)
...
# ####---------------程序初始化-------------
    ...
    def set_camera_param(self, oCtrl, sn):  # 实例及标号#（初始化调用）
        ...
        # oParam = aicfg.CAMERA_PARAMETER#相机参数
    ...
    def save_ai_image(self, data_ai, start_time1, tp_id):
        # data_ai={相机号:[图片路径,点位信息],相机号:[图片路径,点位信息],相机号:[图片路径,点位信息]}
        try:
            self.after_pic_path = {}; will_judge = []  # 置空值
            if self.havebox_path is not None:
                bool_record1 = 0; bool_add_nobox = 0
                bool_record1 = 1 if (self.num_take_pic % 3 == 1) else 0  # 控制存入上传的变量-havebox
                bool_add_nobox = 1 if (self.num_take_pic % 50 == 1) else 0  # 控制存入上传的变量-nobox
                for camera_num, point_data in data_ai.items():  # 分单张图
                    ...
                    if len(point_data_ori) > 0:  # 此图有点信息时
                        ...
                        ###数据库
                        list_6 = [tp_id, self.havebox_path + "/" + path_, start_time1, 1]
                        id6_pic = write_mysql6(list_6)  # 录入图片表
                        cam_num1 = aicfg.CAMERA_DEVICE_POSITION_MAP[camera_num]  # 相机号对应简单编号
                        ...
                        for point_message1 in point_data_ori: #单张图中的每一个点信息
                            if point_message1["score"] >= aicfg.Classification_threshold:
                                level_point = "havebox"
                            else:
                        ...
                        cam_num1 = aicfg.CAMERA_DEVICE_POSITION_MAP[camera_num]
                        self.after_pic_path[cam_num1] = id6_pic
                        if bool_add_nobox > 0:  # 把每隔数次的可用 图片+txt cp 到upload路径
                            copy_path_upload_no = "cp -f " + self.nobox_path + "/" + path_ + " " + self.upload_nobox + "/"
                            os.system(copy_path_upload_no)
