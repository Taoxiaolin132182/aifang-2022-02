...
'''TianYu--claw-1.1--2021-12-03-ARM1'''
'''X总长684.6mm,Y总长(580mm),AB相机实体之间距离607mm，'''
'''镜头到传送带的距离为660mm，提高30mm标定，实际工作距离630mm'''
'''相机AB排布，3颗相机对应的坐标系与PLC抓手X反向，Y同向'''
'''B点世界（0，0）像素坐标对应点，，到与支架前部内框点（近电柜处）某一个固定点距离（763,303.5）'''
'''A() B() C()图中（1224，1024）对应的X值'''
'''AB距离607mm，--重合mm，BC距离596.5mm(由3颗相机拍一组卷尺换算长度得到数据（x=1224时的卷尺读数差）)'''
pxy_add1 = [[156, 500], [1331.5, 503.5], [743, 488]]  # 与支架前部内框点（近电柜处）某一个固定点距离 #123 -acb
# pxy_add1 = [[0, 0], [0, 0], [0, 0]]  # 与支架前部内框点（近电柜处）某一个固定点距离
# pxy_add1_a = [203, 1109]  # 支架与抓手1单位距离(90,820) + 210 是因为Y轴相反：Y=210-y
pxy_add1_a = [0, 0]
