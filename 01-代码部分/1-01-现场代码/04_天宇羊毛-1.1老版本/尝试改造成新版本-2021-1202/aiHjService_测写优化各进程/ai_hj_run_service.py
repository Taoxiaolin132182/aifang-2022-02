...
class AiHjRunService:
    def __init__(self):
        ...
        self.give_cotton_l = [False, -1, -1]  # 给棉信号，[触发读取, 0, 1] --2021_1129
        ...
        self.pic_queue1 = [[False, False, queue.Queue(maxsize=3)], [False, False, queue.Queue(maxsize=1)]]
        self.change_direction = [False, 0]  # 羊毛出毛的转向控制信号
    ...
    def keep_connection_to_PLC(self):
        ...
        name29 = aicfg.PLC_address.CHANGE_OK  # 通知羊毛出毛端 转向
        ...
        while True:
            try:
                ...
                if self.change_direction[0]:
                    self.change_direction[0] = False
                    input_num = [self.change_direction[1]]
                    self.PLC_ctrl[aicfg.Run_choose[1]].write_bit_data(name29, input_num)
                ...
                if aicfg.PLC_ctrl_choose[3] == 1:
                    ...
                    if self.bool_up_to_standard:  # 异纤数量-达标信号
                        ...
                        self.give_cotton_l = [False, -1, -1]  # 重置给棉信号

                    '''读取给棉信号'''
                    if self.give_cotton_l[0]:
                        self.give_cotton_l[0] = False
                        ig = 2 if self.give_cotton_l[1] == 0 else 1  # 当有了0 后，去读取 1
                        if self.give_cotton_l[1:] == [0, -1]:
                            logger8.info("首先取到给棉信号初始值(0)")
        ...
        while True:
            try:
                ...
                if not bool_take_pic:  # 拍照成功-存图，
                    ...
                    if self.pic_queue1[0][0]:
                        self.pic_queue1[0][0] = False
                        for num_cam in range(len(aicfg.Camera_id)):
                            ...
                            if not self.pic_queue1[0][2].full():
                                self.pic_queue1[0][2].put(oriImg)
                        self.pic_queue1[0][1] = True
            except Exception as e:
...
######2021-05-19----喷气部分------------#########
    ...
    def thread_recevived_signal(self):
        ...
        exist_thins = [0, 0, [0, 0, 0]]  # 有无羊毛的计数: [0]有羊毛临时计数,[1]无羊毛，[2]:状态位
        while True:
            try:
                '''计数限制循环'''
                ...
                if count_send[0] % 12 == 3:
                    self.pic_queue1[0][0] = True  # 触发--把图片放入队列
                if self.pic_queue1[0][1]:  # 当确认返回时
                    self.pic_queue1[0][1] = False
                    num_once_test = 0
                    for pic_i in range(len(aicfg.Camera_id)):
                        if not self.pic_queue1[0][2].empty():
                            pic_list1 = self.pic_queue1[0][2].get()
                            num_once_test += self.search_empty(pic_list1)
                    n_id = 1 if num_once_test == 0 else 0
                    exist_thins[n_id] += 1
                    if exist_thins[0] > 4:
                        exist_thins[2][0] += 1; exist_thins[:2] = [0, 0]
                    if exist_thins[1] > 2 and exist_thins[2][0] > 0:
                        exist_thins[2][1] += 1; exist_thins[:2] = [0, 0]
                    if exist_thins[2][1] > 0 and exist_thins[2][0] > 0:
                        exist_thins[2][2] += 1; exist_thins[2][:2] = [0, 0]
                        send_num = exist_thins[2][2] % 2  # 单次时，等于第一遍，需要转向，M1454= 1
                        self.change_direction = [True, send_num]  # 羊毛出毛的转向控制信号
                        logger8.info("计时序号{}-向PLC发送[{}]-反转--计数列表:{}".format(
                            count_send[0], send_num, exist_thins[2]))
                    logger8.info("计时序号{}--有羊毛计数{}，无羊毛计数{}---计数列表:{}".format(
                        count_send[0], exist_thins[0], exist_thins[1], exist_thins[2]))
        ...
        while bool_run_statu:
            ...
            try:
                ...
                if list_time_run[0] is None:  # 只会在第一次运行中执行
                    ...
                    # 获取plc的给棉信号
                    self.give_cotton_l[0] = True; time.sleep(0.5)
                    if self.give_cotton_l[1] == 0 and self.give_cotton_l[2] == 1:
                        logger8.info("收到PLC传来的给棉信号(先0后1)")
                ...
                if self.if_need_stop == 0:  # 信号停止中
                    ...
                    if list_time_run[0] is not None:  # 只会在第一次运行中执行
                        ...
                        if len_r1 >= record_data1[2]:  # 现在分段时间 大于 限制时长时
                            ...
                            # 判断是否需要达标
                            if count_ok1[0]:
                                if (num_run_time + 1) * record_data1[2] >= record_data1[0] * count_ok1[1]:
                            ...
                            if now_time_length_all >= record_data1[0] * (record_data1[1] - 1):  # >= 每遍时长*最大遍数
                                logger8.info("已经运行{}遍-每遍{}分钟".format(record_data1[1], int(record_data1[0]/60)))
                                logger8.info("发送PLC达标信号")
                                self.bool_up_to_standard = True
                                list_start_stop[1] = 1  # 触发停止
                        else:                   # 不满5分钟时，睡眠跳过
                            # 选择判断 1遍---一般处于刚开始计时阶段(还未满1次间隔时间)：
                            if record_data1[1] == 1:
                                self.bool_up_to_standard = True; list_start_stop[1] = 1  # 触发停止
                ...
                if bool_if_get_num:
                    ...
                    if list_start_stop[0] == 1:  # 开始产生点位后
                        ...
                        if bool_l1:
                            ...
                            count_ok1[0] = True
                            t_end = (num_run_time * record_data1[2]) // record_data1[2] + 1  # 达标时刻所在遍数
                            if num_run_time * record_data1[2] >= record_data1[0] * t_end:  # 现在是否为该遍数的最后一次
                                self.bool_up_to_standard = True
                                list_start_stop[1] = 1  # 触发停止
                            else:
                                count_ok1[1] = t_end
