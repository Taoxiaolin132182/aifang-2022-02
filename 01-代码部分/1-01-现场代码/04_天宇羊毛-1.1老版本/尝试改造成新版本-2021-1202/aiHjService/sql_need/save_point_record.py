...
) ENGINE=InnoDB AUTO_INCREMENT=63 DEFAULT CHARSET=utf8 COMMENT='点位表';
'''
    ...
    def filter(self, aoContext):
        ...
        if aoContext.point_id <= 0:
            ...
            strSql = (
                      ...
                      "(`take_photo_id`,`image_id`,`type`,`ff_color`,`ff_type`,`threshold`,`level`,`speed`,"
            ...
            oValues = (aoContext.take_photo_id, aoContext.image_id, aoContext.type, aoContext.ff_color,
                       aoContext.ff_type, aoContext.threshold, aoContext.level, aoContext.speed,
            ...
            strSql = (
                      ...
                      "SET `take_photo_id`=%s,`image_id`=%s,`type`=%s,`ff_color`=%s,`ff_type`=%s,`threshold`=%s,"
                      "`level`=%s,`speed`=%s,`point_xmax`=%s,`point_ymax`=%s,`point_xmin`=%s,`point_ymin`=%s,"
                      "`point_xcenter`=%s,`point_ycenter`=%s,"
