                ...
                if self.bool_xfz:
                    ...
                    if zhua_a != 0:  # 锁定成抓手1/2（起码有一个抓手空闲）
                        ...
                        if zhua_a == 1:
                            ...
                            x = x + aicfg.X1_LEN1[num_cam]
                            y = y - 0.25*speed_bmq1 + aicfg.Y1_LEN1[num_cam]  # 抓手1现场补偿值加大0mm,-5mm(加了连接板5mm)
                            lt1 = aicfg.T1_LEN1[num_cam]
                            ...
                            if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                                ...
                                end_time = current_time + (y/speed_bmq1 - lt1 - (arm_time_now - start_time))*1000
                        ...
                        # --------------------------------------------------------------------------------
                            ...
                            zhua_b = 0  # 一但进入判断，就解锁（不论抓不着还是去抓取）
                            x = x + aicfg.X2_LEN1[num_cam]
                            y = y + 607 - 0.25*speed_bmq1 + aicfg.Y2_LEN1[num_cam]  #抓手2与抓手1相距607mm+5mm(加了连接板5mm)
                            lt1 = aicfg.T2_LEN1[num_cam]
                            current_speed_x = 520  # mm/s
