# coding=utf-8
'''(2022-02-22) - (张家港天宇1.1-老版本) - (2抓手) - (ARM2) 模组配置'''

'''此模块特征(不同主机，会有不同)'''
ARM_number = 2  # ARM 2号
Path_upload = "zjgty_old1.1_arm2_"  # 上传图片路径标记
Code_version = 4.2
Check_exist_wool = True  # True:需要检测羊毛有无， False:不检测羊毛有无
Mechine_function = "claw"  # 设备模式  claw - 抓手模式， gas - 喷气模式
If_need_PLC = False  # True:正常使用PLC， False:屏蔽PLC相关功能
...
If_upload_nobox = True  # True: 上传nobox， False: 不传
...
Frame_time = 0.95  # 拍照间隔时长(秒) - 生产者循环中
Update_times = 120  # 上传频率 次数  (上传快：6  || 慢：60)
...
X1_LEN1 = [223, 223, 223]             # 抓手1的X轴距离补偿
Y1_LEN1 = [1561, 1561, 1561]             # 抓手1的Y轴距离补偿
T1_LEN1 = [0.15, 0.15, 0.15]       # 抓手1的下抓时间补偿(减去的时间，值越大，下抓时刻越早)

X2_LEN1 = [228, 228, 228]             # 抓手2的X轴距离补偿
Y2_LEN1 = [2218, 2218, 2218]       # 抓手2的Y轴距离补偿
T2_LEN1 = [0.45, 0.45, 0.45]       # 抓手2的下抓时间补偿

X3_LEN1 = [0, 0, 0]              # 抓手3的X轴距离补偿
Y3_LEN1 = [1500, 1500, 1500]     # 抓手3的Y轴距离补偿
T3_LEN1 = [0.6, 0.6, 0.6]        # 抓手3的下抓时间补偿

X4_LEN1 = [0, 0, 0]               # 抓手4的X轴距离补偿
Y4_LEN1 = [2100, 2100, 2100]      # 抓手4的Y轴距离补偿
T4_LEN1 = [0.6, 0.6, 0.6]         # 抓手4的下抓时间补偿

Speed_of_csd = 220  # 传送带速度 600mm/s(预设)
Speed_rate = 0.0653  # 传送带速度比率：张家港1.1  33.82hz -2022-0222
...
AI_model_choose = 2
AI_color_label = True  # True : 使用异纤颜色分类模型，False ： 不使用(一般不使用)
# AI_model_cfg_path = "/mnt/data/data/aimodel/20210813_yangrong_model/wool_jetpack44_v1_0_trt_config.json"
AI_model_cfg_path = "/mnt/data/data/aimodel/wxmf_sansimian_20220215/cotton_jetpack44_v1_0_trt_config.json"
...
# Camera_id = [["35024027", "A", "1"],
#              ["35024015", "C", "2"],
#              ["35024028", "B", "3"]]  # 张家港1.1-ARM2 对应相机号
Camera_id = [["13914456", "A", "1"],
             ["39024066", "C", "2"],
             ["39024079", "B", "3"]]  # 蓝本机对应相机号
# Camera_id = [["13914456", "A", "1"],
...
PLC_ip_frist = '192.168.1.252'  # 第一抓取模组  --ssc-1.1-0915
PLC_ip_second = '192.168.1.x'  # 第二抓取模组  --ssc-1.1-0915
