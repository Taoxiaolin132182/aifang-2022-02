...
def infer(request):
    ...
    print(f"AI 返回结果：{json.dumps(retObj)}")
    return HttpResponse(json.dumps(retObj))
...
def doInfer(imagePath):
    ...
    if isModelForward == False:
        ...
        # model_detection = DetectionModelForward("/opt/model/20200805_cotton_trt_model_fp16_v2.1")
        model_detection = DetectionModelForward("/opt/model/yolov3_cotton_fp16_detection.pb")
    # 改成：调用模型进行推断
...
def callAiModel(model_detection, imagePath, model_classify=None, labels_name=None):
    ...
    for img_path_id in range(len(img_paths)):
        ...
        path = subdir + ".jpg"
        if not os.path.exists("/media/data/Data/image/nobox/20200430"):
            os.makedirs("/media/data/Data/image/nobox/20200430")
        if not os.path.exists("/media/data/Data/image/havebox/20200430"):
            os.makedirs("/media/data/Data/image/havebox/20200430")
        if len(boxes_return):
            cv2.imwrite(os.path.join("/media/data/Data/image/havebox/20200430", path_), img_org)
        else:
            cv2.imwrite(os.path.join("/media/data/Data/image/nobox/20200430", path_), img_org)
