...
def check_paths(path1):
    ...
    if now_path1 == should_be_path:
        ...
        bool_new_path = False
    print("所在路径为目标路径：{}".format(bool_new_path))
...
def start():
    ...
    bool_r = check_paths(path)
    run("test_run.py", "/opt/logs/txl01/ctrl.log", bool_r)


def check_process_exist(process_key):
    print("check the process if exist %s" % process_key)
    name1 = "python3 " + process_key
    child = subprocess.Popen(["pgrep", "-f", name1], stdout=subprocess.PIPE, shell=False)
    pid = child.communicate()[0]
    name2 = "python3 " + should_be_path + process_key
    child2 = subprocess.Popen(["pgrep", "-f", name2], stdout=subprocess.PIPE, shell=False)
...
def run(execute_name, output_file, bool_res):
    ...
    if bool_res:
        ...
        cmd_str = "echo %s|sudo -S nohup python3 %s > %s 2>&1 &" % (sudo_pwd, should_be_path + execute_name, output_file)
...
def daemon():
    ...
    global min_process
    while True:
        pid = check_process_exist("test_run.py")
        if pid is None:
