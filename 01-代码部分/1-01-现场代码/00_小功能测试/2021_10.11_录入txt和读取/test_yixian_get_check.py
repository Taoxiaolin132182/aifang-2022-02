    ...
    def ctrl_run_stop(self):
        ...
        while True:
            ...
            try:
                ...
                if bool_ctrl:
                    if self.if_need_stop != 0:  # 此时在运行
                        print("时刻：---停止")
                        self.if_need_stop = 0
                else:
                    if self.if_need_stop != 1:  # 此时停止
                        print("时刻：-- 运行")
                        self.if_need_stop = 1
            except Exception as e:
                ...
                if self.if_need_stop == 0:
                    ...
                    for il in range(len(limit_list)):
                        if (num_time1 > limit_list[il][0]) and (num_time1 < limit_list[il][1]):
                            num_point_now = 1
                    self.num_point[4] += num_point_now
                    time.sleep(1.5)
            except Exception as e:
    ...
    def choose_times_print(self, list_num1=[]):
        ...
        if len(list_num1) == 3:
            ...
            if dividend1 % divisor1 == remainder1:
                if dividend1 != self.num_point[register1]:  # 当在10计数时间段内，确保指执行1次
                    self.num_point[register1] = copy.deepcopy(self.photo_n)
                    bool_output1 = True
        return bool_output1

    def test_print(self):
        list_num_need = [10, 2, 2]
        list_dividend1 = [6, 11, 12, 14, 22, 26, 32, 36, 42]
        for num1 in range(len(list_dividend1)):
            self.photo_n = list_dividend1[num1]
            res1 = self.choose_times_print(list_num_need)
            print("数字 {} 的结果为： {}".format(self.photo_n, res1))
            time.sleep(1)

    '''主函数部分'''
    def main_func1(self):
        # th1 = threading.Thread(target=self.ctrl_run_stop, args=(), name="ctrl_run_stop")
        # th1.start()
        # th2 = threading.Thread(target=self.produce_point, args=(), name="produce_point")
        # th2.start()
        # th3 = threading.Thread(target=self.record_point_count, args=(), name="record_point_count")
        # th3.start()
        self.test_print()
...
if __name__ == "__main__":
    # service_m = Test_func_code()  # 实例化
    # service_m.main_func1()
    # test1()
