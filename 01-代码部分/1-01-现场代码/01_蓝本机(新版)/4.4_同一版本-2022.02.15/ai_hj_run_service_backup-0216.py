            ...
            if zhua_a == 0:  # 锁定抓手1/2/3/4（起码有一个抓手空闲且对应队列有值）
                ...
                if aicfg.claw_section:
                    ...
                    if (zhua_a == 1) or (zhua_a == 3):
                        x_p = randint(aicfg.Min_value_X, aicfg.X_choose_claw)
                    else:
                        x_p = randint(aicfg.X_choose_claw, aicfg.Max_value_X)
                else:
                    x_p = randint(aicfg.Min_value_X, aicfg.Max_value_X)
                x = x_p + X_l_cfg[zhua_a - 1][0]
                '''发送抓取信息'''
                if bool_send_time:
