...
#------------------主体函数------------------------
    ...
    def wait_deal_data1(self, tp_num2):
        ...
        if bool_check_ai:
            ...
            elif aicfg.Mechine_function == "claw":  # 处理抓手类的点信息
...
#------------------基础函数------------------------
    ...
    def read_batch(self, sign1):  # 初始化时 sign1=Fasle ，
        ...
        sign2 = True  # 默认去读写
        if sign1:
            result_sign = self.PLC_ctrl[1].read_bit_data(aicfg.MELSEC_CODE.SIGN_BATCH, 1)
            if result_sign[0] == 0:  # 当信号位为0，表示无变化时，不去读取大小批次
                sign2 = False
        logger.info('读取批次号--PLC通讯--2')
        if sign2:
            result1_ascii = self.PLC_ctrl[1].read_word_data(aicfg.MELSEC_CODE.BIG_BATCH, 5)
            result1_char1 = self.ascii_to_char(result1_ascii)
            result2_ascii = self.PLC_ctrl[1].read_word_data(aicfg.MELSEC_CODE.SMALL_BATCH, 8)
            ...
            try:
                ...
                if zhua_a != 0:  # 锁定抓手1/2/3/4（起码有一个抓手空闲且对应队列有值）
                    ...
                    if zhua_a == 1:
                        x += aicfg.X1_LEN1
                        y += aicfg.Y1_LEN1
                        lt1 = aicfg.T1_LEN1  # 抓手下抓的过程时间--需要在总时间内扣除
                        if x < x_limit_list[0]:
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (
                                        y / speed_bmq1 - aicfg.T1_LEN1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            print(
                    ...
                    if zhua_a == 2:
                        x += aicfg.X2_LEN1
                        y += aicfg.Y2_LEN1
                        lt1 = aicfg.T2_LEN1  # 抓手下抓的过程时间--需要在总时间内扣除
                        if x < x_limit_list[0]:
                            current_speed_x = x_speed_list[0]
                        elif x > x_limit_list[1]:
                            current_speed_x = x_speed_list[2]
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (
                                    y / speed_bmq1 - aicfg.T2_LEN1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            logger4.info(
                    ...
                    if zhua_a == 3:
                        x += aicfg.X3_LEN1
                        y += aicfg.Y3_LEN1
                        lt1 = aicfg.T3_LEN1  # 抓手下抓的过程时间--需要在总时间内扣除
                        if x < x_limit_list[0]:
                            current_speed_x = x_speed_list[0]
                        elif x > x_limit_list[1]:
                            current_speed_x = x_speed_list[2]
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (
                                    y / speed_bmq1 - aicfg.T3_LEN1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            print(
                    ...
                    if zhua_a == 4:
                        x += aicfg.X4_LEN1
                        y += aicfg.Y4_LEN1
                        lt1 = aicfg.T4_LEN1  # 抓手下抓的过程时间--需要在总时间内扣除
                        if x < x_limit_list[0]:
                            current_speed_x = x_speed_list[0]
                        elif x > x_limit_list[1]:
                            current_speed_x = x_speed_list[2]
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (
                                    y / speed_bmq1 - aicfg.T4_LEN1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            print(
