...
'''(2021-07-21) - (试生产) - (2.0+1.0+4抓手) - (ARM2) 模组配置'''


'''此模块特征(不同主机，会有不同)'''
ARM_number = 2  # ARM 2号
Path_upload = "ssc_1.1_arm2_"  # 上传图片路径标记
# Mechine_function = "gas"
Mechine_function = "claw"
If_need_PLC = False  # True  、 False

Bool_sleep_produce = False  # 生产者循环中停顿时间--开启变量(True:执行停顿)
Sleep_time_produce = 1.75  # 生产者循环中停顿时间
Bool_use_claw = True  # True:正常，False:屏蔽抓手
List_claw_statu = [1, 1, 1, 1, 1, 1, 1, 1]
Need_time_PLC = 1  # ARM2连接的是老的PLC，用[1]
'''数据相关'''
# 程序选择AI返回的label种类
# 强制指定：1-3 ，大于10 则读取PLC选项
Choose_label = 3  # 1,异纤  2，异纤+脏棉 3,异纤+脏棉+mpg 4, 。。。+ yangshi
#  --{"yixian": [[0.32, 0.4], [800, 800]]}  --{类别：[[分数基础值，划分值],[面积基础值，划分值]]}
Choose_type = [
    [{"yixian": [[0.32, 0.4], [800, 800]]}],
    [{"shenhuangmao": [[0.25, 0.5], [800, 2500]]}, {"zangmian": [[0.1, 0.5], [800, 2500]]}],
    [{"yangshi": [[0.5, 0.5], [10000, 10000]]}, {"mpg": [[0.15, 0.25], [800, 800]]}]]
# Limit_num_once =
Speed_of_csd = 500  #传送带速度 500mm/s
# 同帧-去重的距离限制
Length_same_point_x = 80
Length_same_point_y = 35
#PLC抓手轴速度
speed_to_x_list = [500, 1000]  # X值分区
# speed_PLC_X = [1900, 2140, 2325]  # X值分区对应的速度值 mm/s
...
PQ_T = 0.2
# 抓手偏移量
X1_LEN1 = 35          # 抓手1的X轴距离补偿
Y1_LEN1 = 0            # 抓手1的Y轴距离补偿
T1_LEN1 = 0.2  # 抓手1的下抓时间补偿(减去的时间，值越大，下抓时刻越早)

X2_LEN1 = 25           # 抓手2的X轴距离补偿
Y2_LEN1 = 583      # 抓手2的Y轴距离补偿
T2_LEN1 = 0.2  # 抓手2的下抓时间补偿

X3_LEN1 = 40            # 抓手3的X轴距离补偿
Y3_LEN1 = 1020            # 抓手3的Y轴距离补偿
T3_LEN1 = 0.35          # 抓手3的下抓时间补偿

X4_LEN1 = 40            # 抓手4的X轴距离补偿
Y4_LEN1 = 1603            # 抓手4的Y轴距离补偿
T4_LEN1 = 0.39           # 抓手4的下抓时间补偿

# Classification_threshold = 0.3  # 异纤分类阈值：优先和疑似
# Suspected_threshold = 0.1  # 分类阈值
# Other_threshold = 0.1  # 分类阈值

Speed_rate = 0.0742  # 传送带速度比率：试生产1.1  20hz
Min_value_X = 300  # 抓取限位X最小 --z
Max_value_X = 1965  # 抓取限位X最小
# Min_area = 800  # AI检测到点的最小面积

'''AI相关'''
# AI模型配置文件路径
#AI_model_cfg_path = "/mnt/data/data/aimodel/20201210_model_v2.6/cotton_jetpack44_v1_0_trt_config.json"
# AI_model_cfg_path = "/mnt/data/data/aimodel/wxmf_sansimian_20210621/cotton_jetpack44_v1_0_trt_config.json"
# AI_model_cfg_path = "/mnt/data/data/aimodel/20201210_model_v2.6_1024/cotton_jetpack44_v1_0_trt_config.json"
# AI_model_cfg_path = "/mnt/data/data/aimodel/20210628_yangrong_model/wool_jetpack44_v1_0_trt_config.json"
AI_model_cfg_path = "/mnt/data/data/aimodel/20210722_yangrong_model/wool_jetpack44_v1_0_trt_config.json"
# AI_model_cfg_path = "/mnt/data/data/aimodel/wxmf_sansimian_20210723/cotton_jetpack44_v1_0_trt_config.json"
AI_save_pic_bool = True
AI_draw_pic_bool = False
Camera_id = [["13914456", "A", "1"],
             ["39024066", "C", "2"],
             ["39024079", "B", "3"]]  # 试生产ARM2 对应相机号
# Camera_id = [["13914456", "A", "1"],
#              ["48024031", "C", "2"],
...
'''PLC相关'''
# mobus server地址
arm1_ip = '192.168.1.110'  # 主机1 IP
arm2_ip = '192.168.1.111'  # 主机2 IP
PLC_ip_main = '192.168.1.254'  # 触摸屏IP(需要通讯时)
PLC_ip_frist = '192.168.1.252'  # 第一抓取模组
PLC_ip_second = '192.168.1.105'  # 第二抓取模组
PLC_port = 10002  # PLC端口号
PLC_ctrl_choose = [0, 1, 1]  # 选择要连接的PLC模块
...
CAMERA_DEVICE_TUPLE = ("13914456", "39024066", "39024079")
CAMERA_DEVICE_POSITION_MAP = {"13914456":"1","39024066":"2","39024079":"3"}
CAMERA_SAVE_IMAGE = {"13914456": "A", "39024066": "C", "39024079": "B"}#左A右C中B
