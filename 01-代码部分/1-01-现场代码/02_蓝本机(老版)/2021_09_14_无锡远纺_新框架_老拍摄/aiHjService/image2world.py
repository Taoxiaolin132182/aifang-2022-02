...
'''shi_shengchan_2021_03_19'''
'''X总长733mm,Y总长(612mm),AB之间图像重合mm,BC之间图像重合mm'''
'''镜头到传送带的距离为734，提高60mm标定，实际工作距离674mm,相机之间625mm'''
'''相机CBA排布，3颗相机对应的坐标系与PLC抓手X相反，Y同向'''
'''B点世界（0，0）像素坐标对应点，，到与支架前部内框点（近电柜处）某一个固定点距离（754,264）'''
'''B(925) A(1553) C(350)图中（1224，1024）对应的X值'''
'''AB距离601mm，BC距离609mm(由3颗相机拍一组卷尺换算长度得到数据（x=1224时的卷尺读数差）)'''
pxy_add1 = [[1382 + 10, 264 + 35], [179, 264], [754, 264 + 30]]#与支架前部内框点（近电柜处）某一个固定点距离
# pxy_add1 = [[0,0],[0,0],[0,0]]#与支架前部内框点（近电柜处）某一个固定点距离
pxy_add1_a = [1820 + 215 -11.8, 740]#支架与固定点单位距离
# pxy_add1_a = [1820 - 11.8, 0]#支架与固定点单位距离
