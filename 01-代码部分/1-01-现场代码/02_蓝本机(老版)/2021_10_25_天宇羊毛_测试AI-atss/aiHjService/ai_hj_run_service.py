...
# ####---------------程序初始化-------------
    ...
    '''测试 老版 aiinfer'''
    def aiinfer_test1(self):
        # 确认路径
        list_real_path1 = []
        pic_save_path1 = "/mnt/data/zzb_test/wool/test/"
        s1_path1 = os.path.exists(pic_save_path1)
        if s1_path1:
            list_file = os.listdir(pic_save_path1)
            for name_file in list_file:
                if not "jpg" in name_file:
                    continue
                print("name_file:{}".format(name_file))
                # 生成格式
                pic_now = pic_save_path1 + name_file
                list_real_path1.append(pic_now)
        # 调用AIinfer
        for iai in range(len(list_real_path1)):
            res = call_ai_service(list_real_path1[iai], 90)  # 调用ai服务，#返回字符串
            print("res[{}]: {}\n".format(iai, res))
            useful_mes1 = res.get("data").get("0")
            print("useful_mes1[{}]: {}\n".format(iai, useful_mes1))
            self.write_point_to_txt1(list_real_path1[iai], useful_mes1)


    '''根据aiinfer 的返回，生成对应的txt'''
    def write_point_to_txt1(self, pic_path2, mess1):
        s1_path2 = os.path.exists(pic_path2)  # 存在该图片
        if s1_path2:
            if isinstance(mess1, dict):
                list_message1 = []
                for idn in range(len(mess1["labels"])):
                    p_res_m1 = {
                        "leftTopX": mess1["boxes"][idn][0], "leftTopY": mess1["boxes"][idn][1],
                        "rightBottomX": mess1["boxes"][idn][2], "rightBottomY": mess1["boxes"][idn][3],
                        "labelType": mess1["labels"][idn],
                        "labelMap": {
                            "level": 0, "score": mess1["scores"][idn]
                        }
                    }
                    list_message1.append(p_res_m1)
                # 开始存信息
                # print("写入文件时的点信息：{}".format(json.dumps(list_message1)))
                txt_pic_name = pic_path2.replace("jpg", "txt")
                with open(txt_pic_name, "w+") as tf2:
                    tf2.write(json.dumps(list_message1))
