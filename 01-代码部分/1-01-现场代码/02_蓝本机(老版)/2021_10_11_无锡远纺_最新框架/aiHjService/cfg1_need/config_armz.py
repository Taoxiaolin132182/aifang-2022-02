# coding=utf-8
'''(2021-09-12) - (试生产-1.1-NO.2) - (4抓手) - (ARM1) 模组配置'''

'''此模块特征(不同主机，会有不同)'''
ARM_number = 1  # ARM 1号
Path_upload = "ssc_1.1.2_arm1_"  # 上传图片路径标记
Bool_tp_old = True  # 是否为老版本拍照： True:老版本拍照， False:新版本拍照
...
Frame_time = 1.4  # 拍照间隔时长(秒) - 生产者循环中
...
List_claw_statu = [11, 11, 1, 1, 1, 1, 1, 1]   # 0:空闲 / 1:忙碌  / >1 不赋值
...
Speed_of_csd = 160  #传送带速度 500mm/s
# 同帧-去重的距离限制
...
X1_LEN1 = [60, 95, 50]             # 抓手1的X轴距离补偿
Y1_LEN1 = [0, 0, 0]             # 抓手1的Y轴距离补偿
T1_LEN1 = [0.6, 0.6, 0.6]       # 抓手1的下抓时间补偿(减去的时间，值越大，下抓时刻越早)

X2_LEN1 = [40, 75, 30]             # 抓手2的X轴距离补偿
Y2_LEN1 = [600, 580, 575]       # 抓手2的Y轴距离补偿
T2_LEN1 = [0.62, 0.65, 0.62]       # 抓手2的下抓时间补偿
...
Speed_rate = 0.0598  # 传送带速度比率：试生产1.1  25hz -2021-0928
Min_value_X = 242  # 抓取限位X最小
Max_value_X = 1942  # 抓取限位X最小
...
Camera_id = [["30124932", "A", "1"],
             ["30124934", "C", "2"],
             ["35024021", "B", "3"]]  # 试生产1.2-ARM1 对应相机号
...
PLC_ip_main = '192.168.1.111'  # 触摸屏IP(需要通讯时)
PLC_ip_frist = '192.168.1.112'  # 第一抓取模组  --ssc-1.1-0915
PLC_ip_second = '192.168.1.x'  # 第二抓取模组  --ssc-1.1-0915
PLC_port = 10002  # PLC端口号
PLC_ctrl_choose = [1, 1, 0]  # 选择要连接的PLC模块
Run_choose = [0, 1, 2]  # [main, frist, second] 若有需要，可更改(如交换了前后组抓手，等)
