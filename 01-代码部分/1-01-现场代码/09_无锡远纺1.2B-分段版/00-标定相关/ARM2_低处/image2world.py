...
'''claw-1.2--2021-11-09-ARM2'''
'''X总长750mm,Y总长(630mm),AB相机实体之间距离495mm'''
'''镜头到传送带的距离为730，提高30mm标定，实际工作距离700mm'''
'''相机AB排布，3颗相机对应的坐标系与PLC抓手X反向，Y同向'''
'''B点世界（0，0）像素坐标对应点，，到与支架前部内框点（近电柜处）某一个固定点距离（540, 270）'''
'''A(828) B(364) C()图中（1224，1024）对应的X值'''
'''AB距离464mm，--重合286mm，BC距离mm(由3颗相机拍一组卷尺换算长度得到数据（x=1224时的卷尺读数差）)'''
pxy_add1 = [[240, 270], [704, 270], [123, 123]]  # 与支架前部内框点（近电柜处）某一个固定点距离 #123 -acb
# pxy_add1 = [[0,0],[0,0],[0,0]]#与支架前部内框点（近电柜处）某一个固定点距离
pxy_add1_a = [0, 0]#支架与抓手1单位距离(90,820) + 210 是因为Y轴相反：Y=210-y

#用中间相机的图给AC相机标外参，补偿值近乎0
'''Y由同一卷尺上同一点的在不同相机上运算结果差确定'''
pxy_add2 = [[300, 0], [300, 0], [300, 0]]#相机之间由于世界坐标换算出现的误差--补偿值（）
