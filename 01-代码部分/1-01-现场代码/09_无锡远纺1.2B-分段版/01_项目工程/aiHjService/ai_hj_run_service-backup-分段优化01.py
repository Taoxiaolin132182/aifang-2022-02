...
######2021-05-19----喷气部分------------#########
    ...
    def consume_claw_injection1(self):
        ...
        zhua_a = 0
        x_limit_list = aicfg.speed_to_x_list  # X值分区
        x_speed_list = aicfg.speed_PLC_X  # X值分区对应的速度值 mm/s
        ...
        while True:
            try:
                ...
                '''根据抓手状态来进行下一步判断'''
                zhua_a = 0  # 每次初始化 抓手选择
                if self.list_claw_state[0] == 0 and self.list_claw_state[1] == 0 and not self.judge_empty_queue([0]):
                    zhua_a = 1  # 进入抓手1比较部分
                elif self.list_claw_state[2] == 0 and self.list_claw_state[3] == 0 and not self.judge_empty_queue([1]):
                    zhua_a = 2  # 进入抓手2比较部分
                else:
                    if self.num_claw_state == 4:  # 当为4抓手模式
                        if self.list_claw_state[4] == 0 and self.list_claw_state[5] == 0 and not \
                                self.judge_empty_queue([0, 2]):
                            zhua_a = 3  # 进入抓手3比较部分
                        elif self.list_claw_state[6] == 0 and self.list_claw_state[7] == 0 and not \
                                self.judge_empty_queue([1, 3]):
                            zhua_a = 4  # 进入抓手4比较部分
                ...
                if zhua_a == 0:  # 锁定抓手1/2/3/4（起码有一个抓手空闲且对应队列有值）
                    ...
                    if zhua_a == 1:
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            # logger7.info("----抓手1运算此点时，点走过的路程：{}mm, \n抓手1计划的时间对应的路程: {}mm".format(
                        ...
                        put_num = 2 if aicfg.claw_section else 1  # 分段时：第三梯队(2) | 正常时：第二梯队(1)
                        self.real_put_point_record(put_num, point_record["level"], point_record)  # 队列梯队2，等级, 点信息
                    ...
                    if zhua_a == 2:
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (y / speed_bmq1 - lt1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            # logger7.info("----抓手2运算此点时，点走过的路程：{}mm, \n抓手2计划的时间对应的路程: {}mm".format(
                            #     spend_length, will_length))
                            self.send_toPLC_list = [zhua_a, int(sec), int(mill_sec), int(x)]
                            self.bool_send_toPLC_claw = True
                            self.num_point[5] += 1  # 抓取ok次数
                            ...
                            if self.num_claw_state == 4:
                                ...
                                point_record["times_judge"] += 1
                                put_num = 3 if aicfg.claw_section else 2  # 分段时：第四梯队(3) | 正常时：第三梯队(2)
                                self.real_put_point_record(put_num, point_record["level"], point_record)  # 队列梯队3，等级, 点信息
                            else:
                                self.num_point[6] += 1
                    ...
                    if zhua_a == 3:
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (y / speed_bmq1 - lt1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            logger7.info("----抓手3运算此点时，点走过的路程：{}mm, \n抓手3计划的时间对应的路程: "
                                         "{}mm".format(spend_length, will_length))
                            self.send_toPLC_list = [zhua_a, int(sec), int(mill_sec), int(x)]
                            self.bool_send_toPLC_claw = True
                            self.num_point[5] += 1  # 抓取ok次数
                    ...
                    if zhua_a == 4:
                        ...
                        if float(spend_length + will_length) < y - lt1 * speed_bmq1:  # 此时，传送带未走到位，才有抓到的条件
                            ...
                            end_time = self.time_plc + (y / speed_bmq1 - lt1 - (self.time_arm - start_time)) * 1000
                            sec, mill_sec = long2time(end_time)  # 毫秒转时间
                            logger7.info("----抓手4运算此点时，点走过的路程：{}mm, \n抓手4计划的时间对应的路程: {}mm".format(
                                spend_length, will_length))
                            self.send_toPLC_list = [zhua_a, int(sec), int(mill_sec), int(x)]
                            self.bool_send_toPLC_claw = True
                            self.num_point[5] += 1  # 抓取ok次数
