...
class AiHjRunService:
    def __init__(self):
        ...
        self.max_times1 = [False, 1]  # 最大遍数 2021-12-02
    ...
    def keep_connection_to_PLC(self):
        ...
        # name27 = aicfg.PLC_address.xxx  # PLC传来的最大遍数
        # name28 = aicfg.PLC_address.xxx  # PLC传来的偏差补偿(起始位)
        ...
        while True:
            try:
                ...
                if aicfg.PLC_ctrl_choose[3] == 1:
                    ...
                    '''取PLC上的最大遍数'''
                    # if self.max_times1[0]:
                    #     self.max_times1[1] = self.PLC_ctrl[aicfg.Run_choose[1]].read_word_data(name27, 1)[0]
                    #     self.max_times1[0] = False
                    '''显示给PLC的循环遍数和分钟数'''
    ...
    def check_takepic_AI_v2(self):
        ...
        for i in range(4):
            try:
                ...
                if i > 2:
                    self.new_add_global_data[1] = 22  # 相机拍照异常--代号
                    self.new_add_global_variables[1] = True
                    logger7.info("自检失败 --退出程序")
                    time.sleep(0.25)
                    # os._exit(0)
...
#------------------主体函数------------------------
        ...
        try:
            if self.havebox_path is not None:
                ...
                for i in range(len(list_pic_name1)):  # 对 每张图片(有图片)
                    pic_path_abs = os.path.join(self.save_pic_aipath, list_pic_name1[i])  # 合成完整 原始图片路径
                    ...
                    if bool_need_move:
                        ...
                        cmd_mv1 = "mv -f " + pic_path_abs + " " + self.havebox_path + "/"  # 每日刷新havebox 路径
                    else:
                        cmd_mv1 = "mv -f " + pic_path_abs + " " + self.nobox_path + "/"  # 每日刷新nobox 路径
    ...
    ''' 单张图片中 点信息提取写入，并记录到全局变量中，等待上传线程中写入 '''
    def upload_msg_with_save(self, dict_p1, pic_name1):
        """
        ...
        try:
            ...
            if bool_upload:  # 有请求，且不超限，才上传
                pic_name_now = copy.deepcopy(pic_name1)  # 防止错乱
                self.new_add_global_data[2].append(pic_name_now)  # 传入图片名
            '''(功能1)创建并写入单图txt'''
            txt_name1 = pic_name1.replace("jpg", "txt")  # 点信息txt文件
            with open(self.havebox_path + "/" + txt_name1, 'w') as tf1:
        ...
        for i in range(80000):
            ...
            for ic in range(self.num_claw_state):  # 根据现在的抓手数量
                ...
                if claw_sta1 == 0:
                    ...
                    break
            '''选择判断--取队列'''
            if zhua_a == 0:  # 锁定抓手1/2/3/4（起码x有一个抓手空闲且对应队列有值）
                time.sleep(0.1);
                continue
            else:
...
######2021-05-19----喷气部分------------#########
    ...
    def count_queue_num(self):
        ...
        count_list = [0] * num_1 * num_2  # num_1:等级数  | num_2:梯队数
            ...
            try:
                ...
                if (not self.new_add_global_variables[2]) and list2_need_sleep[1]:  # 已有反馈，且未执行
                    for pic_name2 in self.new_add_global_data[2]:  # 复制到上传文件夹
                        ...
                        cp_p_upload = "cp -f " + self.havebox_path + "/" + pic_name2 + " " + self.upload_path + "/"
                        cp_p_upload_t = "cp -f " + self.havebox_path + "/" + txt_name2 + " " + self.upload_path + "/"
                ...
                '''在计数循环开始后，每个2秒去触发读取一下给棉启停信号'''
                # if count_send[0] % 4 == 1 and self.give_cotton_l[3]:
    ...
    def read_circulation_record(self):
        ...
        try:
            ...
            txt_name_cycle = self.upload_path + "/circulation_record_message.txt"
    ...
    def write_record_circulation(self, rec_mess2):  # 录入 循环计数信息
        try:
            txt_name1 = self.upload_path + "/circulation_record_message.txt"
    ...
    def record_point_count(self):
        ...
        # while self.max_times1[0]:
        #     time.sleep(0.2)
        # record_data1[2] = int(self.max_times1[1])
        # 读取记录文件
