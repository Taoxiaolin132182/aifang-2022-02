# coding=utf-8
'''(2021-11-08) - (试生产1.2B) - (4抓手分段) - (ARM1) 模组配置'''

'''此模块特征(不同主机，会有不同)'''
ARM_number = 1  # ARM 1号
Path_upload = "wxyf_1.2B_arm1_"  # 上传图片路径标记
...
Frame_time = 0.75  # 拍照间隔时长(秒) - 生产者循环中
...
Choose_label = 2  # 1,异纤  2，异纤+脏棉 3,异纤+脏棉+mpg 4, 。。。+ yangshi
#  --{"yixian": [[0.32, 0.4], [800, 800]]}  --{类别：[[分数基础值，划分值],[面积基础值，划分值]]}
Choose_type = [
    [{"yixian": [[0.38, 0.4], [800, 800]]}],
    [{"serong": [[0.25, 0.5], [800, 2500]]},
     {"zangmian": [[0.32, 0.5], [800, 2500]]},
...
X1_LEN1 = [275, 275, 0]        # 抓手1的X轴距离补偿
Y1_LEN1 = [730, 730, 0]        # 抓手1的Y轴距离补偿
T1_LEN1 = [0.5, 0.5, 0]        # 抓手1的下抓时间补偿(减去的时间，值越大，下抓时刻越早)

X2_LEN1 = [-540, -540, 0]      # 抓手2的X轴距离补偿
Y2_LEN1 = [1360, 1360, 0]        # 抓手2的Y轴距离补偿
T2_LEN1 = [0.5, 0.5, 0]        # 抓手2的下抓时间补偿

X3_LEN1 = [330, 330, 0]         # 抓手3的X轴距离补偿
Y3_LEN1 = [1745, 1745, 0]       # 抓手3的Y轴距离补偿
T3_LEN1 = [0.5, 0.5, 0]         # 抓手3的下抓时间补偿

X4_LEN1 = [-510, -510, 0]       # 抓手4的X轴距离补偿
Y4_LEN1 = [2375, 2375, 0]       # 抓手4的Y轴距离补偿
T4_LEN1 = [0.5, 0.5, 0]         # 抓手4的下抓时间补偿

Speed_of_csd = 540  # 传送带速度 600mm/s(预设)
Speed_rate = 0.3607  # 传送带速度比率：试生产1.2B  16hz
Min_value_X = 30  # 抓取限位X最小(基于传送带边缘)
Max_value_X = 1170  # 抓取限位X最大(基于传送带边缘)
time_x_will = 0.15   # 抓手轴X方向运动的时间补偿，可能通讯有延时或是加减速耗时
light_value = 91  # 最低光源亮度值(小于该值代表光源亮度异常)
UV_threshold = 130  # 紫光模式识别的阈值

claw_section = True  # True：抓手分段  || False：抓手一致
X_choose_claw = 600  # 通过X值来决定给哪个抓手来抓(基于传送带边缘)  2021-11-04
Judge_queue = [0, 1]  # [x小于阈值时放入第一梯队，x大于等于阈值时放入第二梯队] 2021-11-04
Throw_location = [0, 1000, 0, 1000]  # 各抓手的抛料位的x值[1号抓手，2号，3号，4号，]
...
AI_model_cfg_path = "/mnt/data/data/aimodel/wxmf_sansimian_20220203/cotton_jetpack44_v1_0_trt_config.json"
...
Camera_id = [["39024083", "A", "1"],
             ["48024035", "B", "2"]]  # 试生产1.2-ARM1 对应相机号
...
PLC_ip_frist = '192.168.1.121'  # 第一抓取模组  --ssc-1.2
PLC_ip_second = '192.168.1.122'  # 第二抓取模组  --ssc-1.2
PLC_ip_third = '192.168.1.125'  # 添加的出棉机控制PLC  --ssc-1.2 #2021-10-15
PLC_port = 10002  # PLC端口号
PLC_ctrl_choose = [0, 1, 1, 1]  # 选择要连接的PLC模块
