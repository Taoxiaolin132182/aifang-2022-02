...
'''X总长767mm,Y总长未测量(641mm估计),AB之间图像重合mm,BC之间图像重合mm'''
'''镜头到传送带的距离为660，提高50mm标定，实际工作距离630mm,相机之间605mm'''
'''相机CBA排布，3颗相机对应的坐标系与PLC抓手X相同，Y同向'''
'''B点世界（0，0）像素坐标对应点，，到与支架前部内框点（近电柜处）某一个固定点距离（761,481）'''
'''AB距离601mm，BC距离609mm(由3颗相机拍一组卷尺换算长度得到数据（x=1224时的卷尺读数差）)'''
pxy_add1 = [[1362,481-50],[152,481],[761,481]]#与支架前部内框点（近电柜处）某一个固定点距离
# pxy_add1 = [[0,0],[0,0],[0,0]]#与支架前部内框点（近电柜处）某一个固定点距离
pxy_add1_a = [1820 + 201, 370]#支架与固定点单位距离
