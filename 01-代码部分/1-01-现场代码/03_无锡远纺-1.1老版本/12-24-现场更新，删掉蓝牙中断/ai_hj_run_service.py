...
class AiHjRunService:
    ...
    def __init__(self):
        ...
        self.record_queue = queue.Queue(maxsize=10) #其他分类队列
        self.record_queue2 = queue.Queue(maxsize=6)  #其他分类队列 - 次队列
        ...
        try:
            ...
            while bool_light:
                ...
                for key in self.cameraCtrlListDic:
                    ...
                    logger.info('ai-infer--001:{}'.format(res))
    ...
    def run_produce_point_data(self):  # 生产者（产生点数据）
        ...
        bool_del_aimodel = True
        self.batch_now = self.batch_cotton[0] + "-" + self.batch_cotton[1]
        # 循环开始
        while True:
            try:
                # self.sleepdown1 = True
                # self.speed2 = 200
            ...
            try:
                ...
                if self.bool_xfz:
                    ...
                    if sleep_status1 == 0:#PLC需要保持为0
                        ...
                        self.record_queue2.queue.clear()
                        time.sleep(3)
                        continue
                    else:
                    ...
                    '''判断已经抓取次数'''
                    if self.currentHomingCount >= 500 and bool_zs2_2 == 0:  # 当抓手1运动计数超过次数，让抓手回原点
                        ...
                        time.sleep(0.1)
                        continue
                    if self.currentHomingCount2 >= 500 and bool_zs1_2 == 0:  # 当抓手1运动计数超过次数，让抓手回原点
                    ...
                    if zhua_a != 0:  # 锁定成抓手1/2（起码有一个抓手空闲）
                        ...
                        if zhua_a == 1:
                            ...
                            if point_record["level"] == 1:
                                ...
                                num_OK, num_NG = self.real_put_point_record(self.record_queue2, point_record,
                                                                               "其他-次")
    ...
    def judge_empty_queue(self):
        return self.record_queue.empty() and self.record_queue2.empty() and \
               self.fiber_queue.empty() and self.fiber_seemingly_queue.empty() and \
               self.fiber_queue_assit.empty() and self.fiber_seemingly_queue_assit.empty()

    def judge_empty_main_queue(self):
        return self.record_queue.empty() and self.fiber_queue.empty() and self.fiber_seemingly_queue.empty()
            ...
            if self.fiber_queue.empty():
                if self.record_queue.empty():
                    if self.fiber_seemingly_queue.empty():
                        ...
                        logger4.info('抓手1取到 异纤-疑似-主队列数据 {}'.format(rePP1))
                else:
                    rePP1 = self.record_queue.get()
                    logger4.info('抓手1取到 其他分类-主队列数据 {}'.format(rePP1))
            else:
                rePP1 = self.fiber_queue.get()
                logger4.info('抓手1取到 异纤-优先-主队列数据 {}'.format(rePP1))
        else:
            if self.fiber_queue_assit.empty():
                if self.fiber_queue.empty():
                    if self.record_queue2.empty():
                        if self.record_queue.empty():
                            if self.fiber_seemingly_queue_assit.empty():
                                if self.fiber_seemingly_queue.empty():
                                    rePP1 = None
                                else:
                                    rePP1 = self.fiber_seemingly_queue.get()
                            ...
                            rePP1 = self.record_queue.get()
                            logger4.info('抓手2取到 其他分类-主队列数据 {}'.format(rePP1))
                    else:
                        rePP1 = self.record_queue2.get()
                        logger4.info('抓手2取到 其他分类-次队列数据 {}'.format(rePP1))
                else:
                    rePP1 = self.fiber_queue.get()
...
# ####---------------相机拍照/相机回调/图片存放-------------
    ...
    def processAiData(self, res_data, position):  # 返回像素点坐标，相机位置，世界坐标的字典 的列表#（生产者调用）
        ...
        if len(data_label) > 0:  # 为空列表时，可以直接返回 arr = []
            ...
            data_ff_color = res_data["ff_color"]
            # data_ff_type = res_data["ff_type"]
            ...
            for i_point in range(len(data_label)):
                ...
                if data_label[i_point] in self.list_zh1[int(self.choose_model1)]: # 当 label 是在PLC传来的分类列表中
                    ...
                    #         continue
                    # 再判断 点的分类的等级
                    # 1:异纤-优先，2其他分类，3异纤-疑似
                    if data_label[i_point] == "yixian":
                        if float(data_score[i_point]) >= aiCottonConfig.Classification_threshold:
                            point_level = 1 #1:异纤-优先，2其他分类，3异纤-疑似
                            self.ai_point_record.fiber_priority += 1  # 计数 初步筛选 AI-异纤 优先
                        else:
                            point_level = 3
                            self.ai_point_record.other_label += 1  # 计数 初步筛选 AI-其他异常点
                    else:
                        point_level = 2
                        self.ai_point_record.fiber_seemingly += 1  # 计数 初步筛选 AI-异纤 疑似
                    if data_ff_color[i_point] == "white":
                        data_ff_color[i_point] = "other"
                    map = {"position": position, "label": data_label[i_point], "score": round(data_score[i_point], 5),
...
# ####---------------程序初始化-------------
    ...
    def save_ai_image(self, data_ai, start_time1, tp_id):  # data_ai={相机号:[图片路径,点位信息],相机号:[图片路径,点位信息],相机号:[图片路径,点位信息]}
        ...
        if self.havebox_path is not None:
            ...
            if len(self.batch_cotton[0]) + len(self.batch_cotton[1]) > 0:
            ...
            for camera_num, point_data in data_ai.items():  # 分单张图
                ...
                if len(point_data_ori) > 0:  # 此图有点信息时
                    ...
                    ###数据库
                    # batch_now = self.batch_cotton[0] + "-" + self.batch_cotton[1]
                    list_6 = [tp_id, self.havebox_path + "/" + path_, start_time1, 1, self.batch_now]
                    ...
                    id6_pic = 1234  # 2021-08-31 应 金明需求，nobox的image数据不存入，因逻辑后面要检测len,这步要加假数据
                    # print("图片表ID：{}".format(id6_pic))
