...
def search_process(pro_name, bool_ori=True):
    now_id = None
    cmd_str1 = "pgrep -f {}".format(pro_name)
    pid1 = os.popen(cmd_str1)
    message1 = pid1.readlines()  # type: list
    print("共检测到的pid:{}".format(message1))
    if len(message1) > 1:
        list_num = []; res_list1 = [9999, 0, 0]
        for i in range(len(message1)):
            list_num.append(int(message1[i].replace("\n", "")))
        if len(list_num) == 2:
            now_id = str(max(list_num))
        elif len(list_num) == 3:
            for iq in range(len(list_num)):
                for ip in range(iq + 1, len(list_num)):
                    cha1 = abs(list_num[iq] - list_num[ip])
                    if res_list1[0] > cha1:
                        res_list1 = [cha1, list_num[iq], list_num[ip]]
            now_id = str(max(res_list1[1:]))
        print("process id :{}".format(now_id))
    return now_id
