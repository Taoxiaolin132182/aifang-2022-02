...
class model_atss_detection:
    def __init__(self, model_config):
        ...
        self.classify_input_class = model_config["model_atss_detection"]["classify_input_class"]
        self.use_classify = model_config["model_atss_detection"]["use_classify"]
        if self.use_classify:
            self.model_classification = model_classification(url, model_config["model_classification"])
    ...
    def aiInfer(self, oriimg, img, img_name):
        ...
        if self.use_classify:
            for id, each_label in enumerate(labels_return):
                if len(self.classify_input_class) == 0 or (each_label in self.classify_input_class):
                    box = boxes_return[id]
                    img_cut = oriimg[int(box[1]):int(box[3]), int(box[0]):int(box[2]), :]
                    out_label, out_score = self.model_classification.aiInfer(img_cut)
...
class model_classification:
    def __init__(self, url, model_config):
        self.model_config = model_config
        self.url = url
        self.image_input_size = self.model_config["image_input_size"]
        self.model_name = self.model_config["model_name"]
        self.class_name_score_factors = self.model_config["class_name_score_factors"]
        self.class_name = list(self.class_name_score_factors.keys())
        self.model_classfy = TritonClient(self.model_name, model_version='1', url=self.url, shared_memory=False)
        # frist infer
        for i in range(self.model_config["frist_infer_num"]):
            img_gj_ori = np.zeros(
                (1, 3, self.model_config["image_input_size"][1], self.model_config["image_input_size"][0]),
                dtype=np.float32)
            _ = self.model_classfy.inference(img_gj_ori)
    ...
    def dataProcess(self, img):
        '''
        数据处理
        目前就只有resize操作，归一化的操作已经放在模型里面了
        '''
        img_resize = cv2.resize(img, (self.image_input_size[1], self.image_input_size[0]))
        return img_resize

    def aiInfer(self, img):
        '''
        aiIner
        img:未处理的BGR图像（摄像头直接读到的图像为BGR）
        '''
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = self.dataProcess(img)
        input_data = torch.Tensor(img).permute([2, 0, 1]).unsqueeze(0).numpy()
        out_data = self.model_classfy.inference(input_data)
        out = out_data['output_data'][0]
        # output = sigmoid(out)
        output = self.softmax(out)
        scores_return, labels_return = [], []
        if not math.isnan(output[0]):
            label = self.class_name[int(output.argmax())]
        else:
            label = self.class_name[-1]
