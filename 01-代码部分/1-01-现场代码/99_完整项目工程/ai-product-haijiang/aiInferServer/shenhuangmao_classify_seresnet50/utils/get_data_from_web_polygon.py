...
data_save_path_train = "./data/my_data/20201202_wool_data_train_add.txt"
...
batch_list = []
for batch in open("./data/my_data/20201126_wool_data_batch_train_add.txt").readlines():
    batch_list.append(batch.split("\n")[0])

batch_list = ["20201129_wool_select_0", "20201129_wool_select_2"]
...
for batch in batch_list:
    batch = batch.split("\n")[0]
    # print(batch)
    data = requestWithSign("aiadminapi/GetImageRecord/listByWhere", {"batchNo":batch,"pageSize":10000,"belongBusiness":project_name,"handlerStatus":2,"status": 1})#"handler_status":2  "handlerStatus":2, 2是审核通过， 1是标注完成 "status": 1 #（审核）
    # str = json.loads(html)
    # list_key = []
    # print(data)
    if data is None:
