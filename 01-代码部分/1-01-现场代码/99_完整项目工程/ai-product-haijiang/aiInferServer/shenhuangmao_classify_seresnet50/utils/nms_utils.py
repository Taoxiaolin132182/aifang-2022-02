...
def gpu_nms_combine(boxes_1, scores_1, boxes_2, scores_2, num_classes, max_boxes=50, score_thresh_1=0.5, score_thresh_2=0.5, iou_thresh=0.5, area_thresh=1000.):
    """
    Perform NMS on GPU using TensorFlow.
    ...
    boxes_2 = tf.reshape(boxes_2, [-1, 4]) * 2.0  # '-1' means we don't konw the exact number of boxes
...
def gpu_nms_combine_all(boxes_1, scores_1, boxes_2, scores_2, boxes_3, scores_3, boxes_4, scores_4, num_classes, score_thresh_1, score_thresh_2, score_thresh_3, score_thresh_4, area_thresh, max_boxes=50, iou_thresh=0.5):
    """
    Perform NMS on GPU using TensorFlow.
    ...
    boxes_3 = tf.reshape(boxes_3, [-1, 4])  # '-1' means we don't konw the exact number of boxes
    score_3 = tf.reshape(scores_3, [-1, num_classes])

    boxes_4 = tf.reshape(boxes_4, [-1, 4]) * 2.0  # '-1' means we don't konw the exact number of boxes
    score_4 = tf.reshape(scores_4, [-1, num_classes])

    # Step 1: Create a filtering mask based on "box_class_scores" by using "threshold".
    mask_1 = tf.greater_equal(score_1, tf.constant(score_thresh_1))
    mask_2 = tf.greater_equal(score_2, tf.constant(score_thresh_2))
    mask_3 = tf.greater_equal(score_3, tf.constant(score_thresh_3))
    mask_4 = tf.greater_equal(score_4, tf.constant(score_thresh_4))

    # Step 2: Do non_max_suppression for each class
    boxes_combine = tf.concat([boxes_1, boxes_2, boxes_3, boxes_4], 0)
    score_combine = tf.concat([score_1, score_2, score_3, score_4], 0)
    mask_combine = tf.concat([mask_1, mask_2, mask_3, mask_4], 0)
    ...
    iou = broadcast_iou(mg_box_xy, mg_box_wh, yx_box_xy, yx_box_wh)
    best_iou = tf.reduce_max(iou, axis=-1)
    object_mask = tf.cast(best_iou < 0.3, tf.float32)
    # object_mask = tf.expand_dims(object_mask, -1)
    #
    # boxes = tf.concat(boxes_list, axis=0)
    # score = tf.concat(score_list, axis=0)
    ...
    boxes = tf.boolean_mask(boxes_list[0], object_mask)
    score = tf.boolean_mask(score_list[0], object_mask)
    label = tf.boolean_mask(label_list[0], object_mask)
    ...
    return boxes, score, label, boxes_list[0], score_list[0], label_list[0], boxes_list[1], score_list[1], label_list[1]
