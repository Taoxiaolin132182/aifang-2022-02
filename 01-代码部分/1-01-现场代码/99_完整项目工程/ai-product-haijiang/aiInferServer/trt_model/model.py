...
from utils.layer_utils import conv2d, darknet53_body, yolo_block, upsample_layer, yolo_block_prune, darknet53_body_prune
...
class yolov3_trt(object):
    ...
    def predict_2(self, feature_maps):
        ...
        return boxes, confs, probs
    def predict_3(self, feature_maps):
        '''
        Receive the returned feature_maps from `forward` function,
        the produce the output predictions at the test stage.
        '''
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        # conf_pos_mask = tf.reshape(conf_pos_mask, [-1, 1])
        # class_loss_pos = conf_pos_mask * multi_category_focal_loss1(y_true[..., 5:], pred_prob_logits)
        class_loss_pos = conf_pos_mask * focal_loss(pred_prob_logits, y_true[..., 5:])
        ...
        return xy_loss, wh_loss, conf_loss * 100, class_loss * 100
...
class yolov3_prun(object):
    ...
    def forward(self, inputs, is_training=False, reuse=False):
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
                                normalizer_params=batch_norm_params,
                                biases_initializer=None,
                                activation_fn=lambda x: tf.nn.relu(x)):  # tf.nn.leaky_relu(x, alpha=0.1)
                with tf.variable_scope('darknet53_body'):
                    route_1, route_2, route_3 = darknet53_body_prune(inputs)

                with tf.variable_scope('yolov3_head'):
                    inter1, net = yolo_block_prune(route_3, [50, 52, 50, 47, 54, 130])
                    feature_map_1 = slim.conv2d(net, 3 * (5 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    feature_map_1 = tf.identity(feature_map_1, name='feature_map_1')
                    ...
                    inter2, net = yolo_block_prune(concat1, [47, 32, 38, 68, 64, 107])
                    feature_map_2 = slim.conv2d(net, 3 * (5 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    feature_map_2 = tf.identity(feature_map_2, name='feature_map_2')
                    ...
                    _, feature_map_3 = yolo_block_prune(concat2, [55, 38, 24, 41, 46, 96])
                    feature_map_3 = slim.conv2d(feature_map_3, 3 * (5 + self.class_num), 1,
                                                stride=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer())
                    feature_map_3 = tf.identity(feature_map_3, name='feature_map_3')
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        # conf_loss = tf.clip_by_value(conf_loss, -100.0, 100.0)
        # class_loss = tf.clip_by_value(class_loss, -100.0, 100.0)
        return xy_loss, wh_loss, conf_loss * 2.0, class_loss * 1.0
...
class yolov3_trt_prun_new(object):
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        # shape: take 416x416 input image and 13*13 feature_map for example:
        # [N, 13, 13, 3, 1]
        object_mask = y_true[..., 4:5]  # 是否含有目标的概率只有目标.在enchor中心,且iou最大的enchor为1,其余为0
        # shape: [N, 13, 13, 3, 4] & [N, 13, 13, 3] ==> [V, 4]
        # V: num of true gt box
