...
def data_augmentation(img_path, img, boxes, label, img_size, use_label_smothing = False):
    '''
    ...
    '''
    confs = np.ones(label.shape)
    img, boxes, label, confs = my_data_augmentation(img_path, img, boxes, label, img_size, use_label_smothing)

    return img, boxes, label, confs


def process_box(boxes, labels, img_size, class_num, anchors, confs = None):
    '''
...
def parse_data(line, class_num, img_size, anchors, mode):
    ...
    if mode.decode() == 'train':
        img, boxes, labels, confs = data_augmentation(pic_path, img, boxes, labels, img_size, False)
    else:
        img, boxes = resize_image_and_correct_boxes(img, boxes, img_size)
    ...
    # print(labels.shape)
    # print(boxes)
    y_true_13, y_true_26, y_true_52 = process_box(boxes, labels, img_size, class_num, anchors, confs)
