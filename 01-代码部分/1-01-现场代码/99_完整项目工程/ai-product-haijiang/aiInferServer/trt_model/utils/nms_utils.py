...
def gpu_nms_new_prun(boxes, scores, num_classes, max_boxes=50, score_thresh=0.5, iou_thresh=0.5, area_thresh=1.):
    """
    Perform NMS on GPU using TensorFlow.
    ...
    # since we do nms for single image, then reshape it
    #boxes_rate = tf.constant([[5472/2368, 3648/1568, 5472/2368, 3648/1568]], tf.float32)
    boxes = tf.reshape(boxes, [-1, 4]) # '-1' means we don't konw the exact number of boxes
    #boxes = boxes * boxes_rate
    score = tf.reshape(scores, [-1, num_classes])
   ...
   # boxes_wh_min = tf.reduce_min(boxes_wh, axis=1)
   # mask_wh = tf.greater_equal(boxes_wh_min, tf.constant(10.0))
    #boxes = tf.boolean_mask(boxes, mask_wh)
    #score = tf.boolean_mask(score, mask_wh)
    #label = tf.boolean_mask(label, mask_wh)
