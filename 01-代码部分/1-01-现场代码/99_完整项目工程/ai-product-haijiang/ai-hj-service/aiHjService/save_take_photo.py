...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='拍照表';

'''

'''
t_take_photo_record存储上下文
'''
class take_photo_context:
    def __init__(self):
        super().__init__()

        curTimestamp = time.time()
        self.take_photo_id = 0 #拍照编号，key
        self.batch_no = "" #批次
        self.photo_begin_time = curTimestamp #拍照开始时间
        self.photo_end_time = curTimestamp #拍照结束时间
        self.call_ai_begin_time = 0.0 #调用AI开始时间
        self.call_ai_end_time = 0.0 #调用AI结束时间
...
'''
@author maoyanwei
保存到数据库t_take_photo_record
'''
class save_take_photo(db_filter):
    #构造
    def __init__(self):
        super().__init__("save_take_photo")
    ...
    def filter(self, aoContext):
        ...
        if aoContext.take_photo_id <= 0:
            #插入操作
            strSql = (
                      "INSERT INTO `db_cotton_local`.`t_take_photo_record`"
                      "(`batch_no`,`photo_begin_time`,`photo_end_time`,`call_ai_begin_time`,`call_ai_end_time`,"
                      "`create_time`,`update_time`,`state`) "
                      "VALUES (%s,FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),"
                      "FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),%s)"
                     )
            oValues = (aoContext.batch_no, aoContext.photo_begin_time, aoContext.photo_end_time, 
                       aoContext.call_ai_begin_time, aoContext.call_ai_end_time,
                       curTimestamp, curTimestamp, aoContext.state)
        else:
            #更新操作
            strSql = (
                      "UPDATE `db_cotton_local`.`t_take_photo_record` "
                      "SET `batch_no`=%s,`photo_begin_time`=FROM_UNIXTIME(%s),`photo_end_time`=FROM_UNIXTIME(%s),"
                      "`call_ai_begin_time`=FROM_UNIXTIME(%s),`call_ai_end_time`=FROM_UNIXTIME(%s),"
                      "`update_time`=FROM_UNIXTIME(%s),`state`=%s "
                      "WHERE take_photo_id=%s"
                     )
            oValues = (aoContext.batch_no, aoContext.photo_begin_time, aoContext.photo_end_time, 
                       aoContext.call_ai_begin_time, aoContext.call_ai_end_time,
                       curTimestamp, aoContext.state, aoContext.take_photo_id)
        #end if
        ...
        if aoContext.take_photo_id <= 0:
            ...
            # 获取id失败
                ...
                return False
            #end if
            aoContext.take_photo_id = oLastIds[0][0]
        #end if
        #db操作成功
        return True
    #end def
