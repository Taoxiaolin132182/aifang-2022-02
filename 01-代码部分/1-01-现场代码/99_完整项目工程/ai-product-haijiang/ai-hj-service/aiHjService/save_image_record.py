    ...
    def filter(self, aoContext):
        ...
        if aoContext.image_id <= 0:
            ...
            strSql = (
                      ...
                      "(`take_photo_id`,`image_path`,`photo_begin_time`,`photo_end_time`,"
                      "`call_ai_begin_time`,`call_ai_end_time`,`create_time`,`update_time`,`state`) "
                      "VALUES (%s,%s,FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),"
                      "FROM_UNIXTIME(%s),FROM_UNIXTIME(%s),%s)"
                     )
            oValues = (aoContext.take_photo_id, aoContext.image_path, aoContext.photo_begin_time, aoContext.photo_end_time, 
            ...
            strSql = (
                      ...
                      "SET `take_photo_id`=%s,`image_path`=%s,`photo_begin_time`=FROM_UNIXTIME(%s),`photo_end_time`=FROM_UNIXTIME(%s),"
