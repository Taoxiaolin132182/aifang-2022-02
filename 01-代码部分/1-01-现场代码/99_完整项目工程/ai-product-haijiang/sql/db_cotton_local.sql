...
MySQL - 5.7.28-log : Database - db_cotton_local
*********************************************************************
*/
...
) ENGINE=InnoDB AUTO_INCREMENT=313934 DEFAULT CHARSET=utf8 COMMENT='通用上传任务图片状态中间表';
...
CREATE TABLE `t_point_record` (
  ...
  KEY `idx_state` (`state`),
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB AUTO_INCREMENT=48 DEFAULT CHARSET=utf8 COMMENT='点位表';
...
CREATE TABLE `t_take_photo_record` (
  ...
  KEY `idx_create_time` (`create_time`),
  KEY `idx_call_ai_begin_time` (`call_ai_begin_time`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8 COMMENT='拍照表';
