...
BmpImg::BmpImg (const int width,
                const int height) : BmpPixbuf (width, std::abs (height)), _padding(0)
{
...
{	
	...
	const int h = std::abs (header.biHeight);
	const int offset = (header.biHeight > 0 ? 0 : h - 1);
	this->_padding = BMP_GET_PADDING (header.biWidth);
	...
	if ( !_padding )
	{				
		f_img.read((char*)(&this->get_data()[0]), this->get_len_row() * h);
	...
	else
	{
		for (int y = h - 1; y >= 0; y--)
		{
			...
			// Skip the padding
			f_img.seekg (_padding, std::ios::cur);
		}
	}
