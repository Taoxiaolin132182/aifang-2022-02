...
'''claw-1.2--2021-08-17-ARM1'''
'''X总长735mm,Y总长(600mm),AB相机之间距离535mm'''
'''镜头到传送带的距离为705，提高30mm标定，实际工作距离675mm,相机之间mm'''
'''相机ABC排布，3颗相机对应的坐标系与PLC抓手X同向，Y反向'''
'''B点世界（0，0）像素坐标对应点，，到与支架前部内框点（近电柜处）某一个固定点距离（265,220）'''
'''B(996) A(487) C(0)图中（1224，1024）对应的X值'''
'''AB距离509mm，BC距离mm(由3颗相机拍一组卷尺换算长度得到数据（x=1224时的卷尺读数差）)'''
pxy_add1 = [[265, 220], [774, 220], [123, 123]]  # 与支架前部内框点（近电柜处）某一个固定点距离 #123 -acb
# pxy_add1 = [[0,0],[0,0],[0,0]]#与支架前部内框点（近电柜处）某一个固定点距离
# pxy_add1_a = [90, 820 + 210]#支架与抓手1单位距离(90,820) + 210 是因为Y轴相反：Y=210-y
pxy_add1_a = [0, 210]#支架与抓手1单位距离(90,820) + 210 是因为Y轴相反：Y=210-y
#用中间相机的图给AC相机标外参，补偿值近乎0
'''Y由同一卷尺上同一点的在不同相机上运算结果差确定'''
pxy_add2 = [[0,0],[0,0],[0,0]]#相机之间由于世界坐标换算出现的误差--补偿值（）
