...
######2021-05-19----喷气部分------------#########
    ...
    def consume_claw_injection1(self): #消费者-抓手模块
        ...
        self.bool_check_process_PLC = False
        arm_ctrl = aicfg.ARM_number
        # label_list1=[PLC通讯心跳, 监测传送带速度, 读取是否需要暂停, 清空所有队列, 显示抓手状态]
    ...
    def create_or_read_upload_record(self):
        ...
        try:
            ...
            if not bool_name_file:  # 不存在时，创建txt,并写入000
                ...
                if len(context1) == 0:
                    ...
                    for i_label in range(len(self.label_path)):
                        ...
                        logger7.info("已有-点信息记录次数：{}--{}"
                                     "".format(self.label_path[i_label], self.list_num_label[0][i_label]))
                else:
                    context2 = context1[0]
            ...
            logger8.error(f"create_or_read_upload_record  err: {e}")
            time.sleep(0.1)
