# coding=utf-8
'''(2021-10-08) - (仁杰羊毛) - (2抓手) - (ARM2) 模组配置'''

'''此模块特征(不同主机，会有不同)'''
ARM_number = 2  # ARM 2号
Path_upload = "renjie_1.1_arm2_"  # 上传图片路径标记
...
Ctrl_light = True  # 控制灯光变量： True = 需要使用GPIO交替控制， False = 使用 白光常亮模式
...
Frame_time = 1.05  # 拍照间隔时长(秒) - 生产者循环中
...
Y1_LEN1 = [85, 85, 85]             # 抓手1的Y轴距离补偿
T1_LEN1 = [0.46, 0.46, 0.46]       # 抓手1的下抓时间补偿(减去的时间，值越大，下抓时刻越早)

X2_LEN1 = [223, 223, 223]             # 抓手2的X轴距离补偿
Y2_LEN1 = [668, 668, 668]       # 抓手2的Y轴距离补偿
T2_LEN1 = [0.44, 0.44, 0.44]       # 抓手2的下抓时间补偿
...
Speed_of_csd = 170  # 传送带速度 600mm/s(预设)
...
AI_model_cfg_path = "/mnt/data/data/aimodel/20211113_yangrong_model/wool_jetpack44_v1_0_trt_config.json"
...
Camera_id = [["39024091", "A", "1"],
             ["48024043", "C", "2"],
             ["39024073", "B", "3"]]  # 仁杰羊毛1.1-ARM2 对应相机号
...
Camera_param_change = [[[2.10, 10.0], [1300, 20000], [0.41, 0.41]],
                       [[2.10, 10.0], [1200, 20000], [0.41, 0.41]],
                       [[2.10, 10.0], [1400, 20000], [0.41, 0.41]],
                       ["Gain", "Exposure", "Gamma"],]
# 调用ai服务的地址
...
'''PLC相关'''
# mobus server地址
arm1_ip = '192.168.1.110'  # 主机1 IP --ssc-1.1-0915
