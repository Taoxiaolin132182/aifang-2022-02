...
def check_takepic_AI_v2(self):
    ...
    for i in range(20):
        ...
        if len(result_takepic) == len(aicfg.Camera_id):
            bool_take_pic = False
            print("[自检]第{}次到拍照结束用时：{}s".format(str(i + 1), round(time.time() - t2, 4)))  # 0.3458s
        else:
        ...
        if not bool_take_pic:  # 拍照成功
            ...
            # logger5.info("第{}次到AI识别结束用时：{}s".format(str(i + 1), round(time.time() - t2, 4)))  #
            print("[自检]第{}次到AI识别结束用时：{}s".format(str(i + 1), round(time.time() - t2, 4)))  #
            bool_ai_infer = False
            time.sleep(1)
        if not bool_ai_infer:  # ai识别--调用成功
            pass
        # logger5.info("第{}次循环用时：{}s".format(str(i + 1), round(time.time() - t2, 4)))  # 0.3487s
        print("[自检]第{}次循环用时：{}s".format(str(i + 1), round(time.time() - t2, 4)))  # 0.3487s
...
#         if aicfg.CAMERA_ON:
#             oCtrl = tiscamera_ctrl(aicfg.CAMERA_CTRL_WIDTH, aicfg.CAMERA_CTRL_HEIGHT)  # 某个相机实例
#             logger.info(1)
...
def test_readPLC(run_service):
    try:
        ...
        if test_a == 1:
            print("读取传送带速度")
            namesp = aicfg.MELSEC_CODE.CSD_SPEED1
...
def test_takepic(run_service):
    try:
        ...
        '''相机模块自检'''
        logger5.info("-----等待相机模块加载完成")
        time.sleep(3)
        logger5.info("-----Camera部分自检")
        bool_speed = True
        bool_del_aimodel = False
        jn = 0
        cam_err = 0
        cam_A,cam_B,cam_C = 0,0,0
        while True:
            jn += 1
            # time.sleep(1)
            # logger5.info("第{}次拍照".format(jn))
            ...
            # run_service._count_down_latch = count_down_latch(len(aicfg.CAMERA_DEVICE_TUPLE))
            # logger5.info("1")
            tcam_1 = time.time()
            bResult,cam_id = run_service.take_photo(image_path)
            print("第{}次拍照用时：{}".format(jn, round(time.time() - tcam_1, 4)))
            # logger5.info("2")
            if not bResult:
                cam_err += 1
                if aicfg.CAMERA_DEVICE_TUPLE[0] in cam_id:
                    cam_A += 1
                if aicfg.CAMERA_DEVICE_TUPLE[1] in cam_id:
                    cam_C += 1
                if aicfg.CAMERA_DEVICE_TUPLE[2] in cam_id:
                    cam_B += 1
                logger5.info("第{}次拍照,超时：{}".format(jn,cam_id))
                logger5.info("超时次数：{}，相机A：{}，相机B：{}，相机C：{}".format(cam_err, cam_A,cam_B,cam_C))
                if cam_err > 10:
                    logger5.info("退出程序")
                    os._exit(0)
                ...
                if jn % 5 == 0:
                    logger6.info("第{}次--正常拍照取像".format(jn))
                time.sleep(1.1)
                '''AI模块自检————调用ai识别'''
        ...
        logger5.info(f"err---frist_check: {e}")


def run_ai_ctrl(run_service):
    run_service.run_call_ai_func1()
...
if __name__ == '__main__':
    ...
    if run_or_test1 == 1:
        ...
        service.start_run_4_4()  # 调用函数（初始化modbus,及相机）

    else:
        service = AiHjRunService()
        PLC_or_camare1 = 55  # 22：PLC  33：Camera 44:一起初始化
